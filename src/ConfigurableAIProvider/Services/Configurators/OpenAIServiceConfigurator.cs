    ...
    public void ConfigureService(IKernelBuilder builder, ModelDefinition modelDefinition, ConnectionConfig connectionConfig)
    {
        ...
        if (!string.IsNullOrWhiteSpace(connectionConfig.BaseUrl))
        {
            try
            {
                ...
                _logger.LogInformation("Configuring OpenAI client to use custom BaseUrl: {BaseUrl}", connectionConfig.BaseUrl);
            }
            catch (UriFormatException ex)
            {
                _logger.LogError(ex, "Invalid BaseUrl format specified for connection '{ConnectionName}': {BaseUrl}. OpenAI service will use the default endpoint.", 
        ...
        try
        {
            ...
            switch (modelDefinition.EndpointType)
            {
                case EndpointType.ChatCompletion:
                    ...
                    _logger.LogDebug("Added OpenAI Chat Completion: ModelId={ModelId}{CustomEndpoint}", 
