...
>
> 同样也是一个高效的内容录入与信息批处理工具，用于生成流程中的打标、分析等业务，通用易拓展
> 
> 项目基于 Cursor、v0 等 Vibe Coding 应用开发
> 
...
手动批量导入表格数据（注：部分老格式如 xls 的支持不是特别好，通常重新导出或复制粘贴到 xlsx 中能解决问题）

![manual import](Docs/Images/manualimport.png)

基于非结构化文本，亦可使用AI自动识别和提取
...
3. **推理输出优化**: 支持自定义数据结构的推理结果收集和导出、支持 Batch 推理减少 Token 消耗
4. **数据结构支持拓展**: 支持枚举等数据结构，优化部分场景
5. **嵌入功能拓展**: 拓展数据项支持 RAG ，支持提供知识管理服务，同时支持更灵活的推理组合
6. **数据浏览优化**: 可视化和查询相关
7. **整理与重构**: 受限于 Vibe Coding 上下文，部分代码复用较差存在冗余，待清理
8. ······
